...
export async function POST(req: NextRequest, { params }: { params: Promise<{ id: string }> }) {
  ...
  try {
    const { name, trade_name, email, phone, cpf_cnpj, address, city } = await req.json()
    if (!name?.trim()) {
      return NextResponse.json({ error: 'Nome do cliente é obrigatório.' }, { status: 400 })
    }

    const [client] = await query<Client>(
      `INSERT INTO clients (company_id, name, trade_name, email, phone, cpf_cnpj, address, city)
       VALUES ($1, $2, $3, $4, $5, $6, $7, $8) RETURNING *`,
      [id, name.trim(), trade_name ?? null, email ?? null, phone ?? null, cpf_cnpj ?? null, address ?? null, city ?? null]
    )
